...
function load() {
	...
	$.ajax ({
		url: "http://140.114.212.79/api/pagination.php",
		dataType: "json",
...
$(window).scroll(function () {
	...
	if ($(window).scrollTop() + $(window).height() >= $(document).height()) {
		...
		if(last_id >  1080 - curPage[0].value * 36)loadMoreData(last_id);
		console.log('in scroll',curPage,last_id);
	}
});
...
function loadMoreData(last_id) {
	$.ajax(
		{
			url: "/AdvancedWeb-AS3/loadMoreData.php?last_id=" + last_id,
			type: "get",
			beforeSend: function () {
				$('.ajax-load').show();
			}
