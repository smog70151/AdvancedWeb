		...
		if (me.id === "submitBtn"){
			if(lines.length == 0){
				...
				for(var i = 0; i < lines.length; i++){
					var fetch_min = parseInt(lines[i].childNodes[0].childNodes[0].value.split(":")[0]),
						fetch_sec = parseInt(lines[i].childNodes[0].childNodes[0].value.split(":")[1]);
					if(fetch_min == min){
						if(fetch_sec > sec) {
...
$(document).click(function (event) {
  ...
  if($(event.target.parentNode.parentNode).attr('class')=='line-container'){
	x = $(event.target.parentNode.parentNode.children[0].children[0]).attr('value');
	s = x.split(":");
	min = parseInt(s[0]);
	sec = parseInt(s[1]);
	t = min * 60 + sec;
  ...
  // click add-line -> add a new line-box
	...
	x = $(event.target.parentNode.parentNode.parentNode.children[0].children[0]).attr('value');
	s = x.split(":");
	min = parseInt(s[0]);
	sec = parseInt(s[1]);
	t = min * 60 + sec;
